...
export default function LiveStats({ runState, running, totalAttempts, startTime, lastElapsedMs, prefixHexLength }) {
  ...
  if (runState === 'idle') {
    return (
      <div className="rounded-[22px] border border-white/10 bg-slate-900/60 px-4 py-3">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
          <span className="font-medium text-slate-200">Ready</span>
          <span className="text-slate-500">·</span>
          <span className="text-slate-400">Current prefix averages <span className="font-medium text-slate-200">~{formatNumber(expectedAttempts)}</span> attempts.</span>
          <span className="hidden text-slate-500 sm:inline">·</span>
          <span className="hidden text-slate-400 sm:inline">Live stats appear here after Start.</span>
        </div>
      </div>
    )
  }
